...
Usage: python3 Run_triggerStudy1.py <RUN>

Workflow:
  1. Scan Data/<RUN>/raw/ for prad_<RUN>.<FileNo>_raw.root files.
  2. Make sure OutHists/<RUN>/ exists.
  3. For every file, launch  ../TrigVal <RUN> <FileNo>
     in parallel, capped at MAX_PARALLEL=18 concurrent jobs.
  4. When all jobs finish, hadd OutHists/<RUN>/TriggerStudy1_<RUN>_*.root
     into OutHists/<RUN>/TriggerStudy1_<RUN>_All.root.
  5. Delete the individual per-file root files.

Filename convention:
    prad_024014.00015_raw.root   (RUN=24014, FileNo=15)
The 6-digit RUN and 5-digit FileNo widths are preserved when forming output
filenames (e.g. TrigVal_024014_00015.root).
"""
...
MAX_PARALLEL = 18
EXECUTABLE   = "../TrigVal"
DATA_DIR     = "/volatile/hallb/prad/wrightso/X17_Random/"
OUT_BASE     = "../rootOutfiles/RandomClusters/"
RUN_WIDTH    = 6        # zero-pad width for run numbers
FILE_WIDTH   = 5        # zero-pad width for file numbers
# ─────────────────────────────────────────────────────────────────────────
...
def run_trigger_study(run, file_no, loc):
    """
    Launch  ../TrigVal <RUN> <FileNo>  and wait.
    Returns (file_no, returncode, elapsed_seconds, stderr_tail).
    """

    t0 = time.time()
    try:
        file_name = OUT_BASE +"/" + str(run) + "/" + f"TrigVal_0{run}_" + str(file_no)
        command = [EXECUTABLE, "-f", str(loc), "-c", "-R", "-N",  "-b", "-o", str(file_name)]#, str(run), str(file_no)]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run TrigVal in parallel for all recon files of a run, then hadd."
    )
    ...
    pattern = os.path.join(raw_dir, f"prad_{run_str}.*_recon.root")
    ...
    file_pat = re.compile(rf"^prad_{run_str}\.(\d+)_recon\.root$")
    file_nos = []
    for f in files:
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as pool:
        fut_to_fn = {pool.submit(run_trigger_study, run, fn, loc): (fn, loc)
    ...
    # 2. hadd outputs                                                      #
    # ------------------------------------------------------------------ #
    out_pattern    = os.path.join(out_dir, f"TrigVal_{run_str}_*.root")
    individual_outputs = sorted(glob.glob(out_pattern))
    # Exclude the merged file from the inputs in case of a re-run
    merged_path    = os.path.join(out_dir, f"TrigVal_{run_str}_All.root")
